...
* Fixed `Cosmos(Async)Client` instances authenticating with resource tokens (via `CosmosClientBuilder.resourceToken(String)` or `CosmosClientBuilder.permissions(List)`) being considered thin-client (Gateway V2) eligible - such clients now always stay on Gateway V1.
