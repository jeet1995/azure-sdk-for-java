...
/**
 * Thin client E2E tests for change feed operations exercised through the connectivity-probe (implicit)
 * enablement path: incremental change feed for a logical partition and for the full range.
 *
 * <p>These mirror {@link ThinClientChangeFeedE2ETest} but extend
 ...
 */
public class ThinClientEndpointProbeChangeFeedE2ETest extends ThinClientEndpointProbeTestBase {

    @Factory(dataProvider = "clientBuildersWithGatewayAndHttp2")
    public ThinClientEndpointProbeChangeFeedE2ETest(CosmosClientBuilder clientBuilder) {
        super(clientBuilder);
    }
    ...
    public void testThinClientChangeFeedFullRange() {
        ...
        String pk1 = "probeCfFullRange1_" + UUID.randomUUID().toString().substring(0, 8);
        String pk2 = "probeCfFullRange2_" + UUID.randomUUID().toString().substring(0, 8);
    ...
    public void testThinClientChangeFeedPartitionKey() {
        String pkValue = "probeCfPk_" + UUID.randomUUID().toString().substring(0, 8);
