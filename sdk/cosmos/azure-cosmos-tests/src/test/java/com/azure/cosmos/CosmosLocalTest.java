...
public class CosmosLocalTest {

    protected static Logger logger = LoggerFactory.getLogger(CosmosLocalTest.class.getSimpleName());

    @Test(groups = {"simple"})
    public void readItem_WithFakeReadHangForOneReplica() throws JsonProcessingException {

        String testDbId = "testDatabase";
        ...
        Assert.assertEquals(System.getProperty("COSMOS.MIN_CONNECTION_POOL_SIZE_PER_ENDPOINT"), "3");
...
//            .multipleWriteRegionsEnabled(false)
        ...
        CosmosEndToEndOperationLatencyPolicyConfig e2eConfig = new CosmosEndToEndOperationLatencyPolicyConfigBuilder(Duration.ofSeconds(2)).build();
        ...
        Duration workloadExecutionDuration = Duration.ofMinutes(5);
...
//                        .times(1)
    ...
    private void execute(CosmosAsyncContainer container, List<Pair<String, String>> idAndPkValPairs, int docCount, AtomicBoolean isStopped, CosmosEndToEndOperationLatencyPolicyConfig e2eConfig) {
        while (!isStopped.get()) {
            try {
                CosmosItemRequestOptions itemRequestOptions = new CosmosItemRequestOptions();
                itemRequestOptions.setCosmosEndToEndOperationLatencyPolicyConfig(e2eConfig);
                Pair<String, String> idAndPkValPair = idAndPkValPairs.get(ThreadLocalRandom.current().nextInt(docCount));
